...
if df_horiz is not None:
    ...
    sns.heatmap(_corr, annot=True, cmap='vlag', center=0, fmt='.2f')
...
if df_horiz is not None and df_type is not None and not df_type.empty:
    ...
    _df_sample = df_horiz.iloc[100:200].copy()
    ...
    for _idx in range(len(_df_sample)):
        # Sinal target no poço horizontal (últimos 30 pontos)
        _end_idx = _df_sample.index[_idx]
        if _end_idx >= w_size:
            ...
            if len(_target) == w_size:
                ...
                if _m_idx != -1:
                    ...
                    _curr_z = _df_sample.iloc[_idx]['Z']
    ...
    _df_sample['Inter_Match_TVT_30'] = matched_inter_tvts
    _df_sample['Inter_Geo_DeltaZ_30'] = delta_inter_zs
else:
    _df_sample = None


# %%
if _df_sample is not None:
    _cols_inter = ['TVT_Error', 'Inclination_Deg', 'Inter_Match_TVT_30', 'Inter_Geo_DeltaZ_30', 'Z']
    _cols_inter = [c for c in _cols_inter if c in _df_sample.columns]

    _corr_inter = _df_sample[_cols_inter].astype(float).corr()
